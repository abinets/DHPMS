class MetricsController < ApplicationController
  ...
  # GET /metrics
  def index
    @pagy, @metrics = pagy(Metric.sort_by_params(params[:sort], sort_direction))
  end
  ...
  def destroy
    @metric.destroy!
    respond_to do |format|
      format.html { redirect_to metrics_path, status: :see_other, notice: "Metric was successfully destroyed." }
  ...
  def metric_params
    params.expect(metric: [ :value, :time ])
  end
end
